...
def test_synthetic_dataloader_batch_shapes_and_dtype() -> None:
    ...
    loader = create_synthetic_dataloader(config, num_sequences=config.batch_size * 3, seed=7)

    batches = list(loader)

    assert len(batches) == 3
    for batch in batches:
        assert batch.shape == (config.batch_size, config.max_seq_len)
        assert batch.dtype == torch.long
        assert torch.all((batch >= 0) & (batch < config.vocab_size))
