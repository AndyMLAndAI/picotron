...
def test_nope_layer_skips_rotary_embedding() -> None:
    model = PicotronDecoderModel(_config(nope_layers=(1,)))
...
def test_default_config_retains_all_rope_behavior() -> None:
    torch.manual_seed(7)
    default_model = PicotronDecoderModel(_config())
    torch.manual_seed(7)
    base_config = _config()
    ...
    explicit_rope_model = PicotronDecoderModel(explicit_rope_config)
