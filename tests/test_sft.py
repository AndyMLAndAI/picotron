...
def test_sft_loads_picotron_checkpoint_and_learns_new_data(tmp_path: Path) -> None:
    ...
    base_config = replace(load_config(config_path), num_epochs=2, checkpoint_interval=100)
    pretrain_sequence = torch.arange(base_config.max_seq_len, dtype=torch.long)
    pretrain_loader = DataLoader(
        _TokenDataset(pretrain_sequence, base_config.batch_size * 4),
        batch_size=base_config.batch_size,
        shuffle=False,
    )
    torch.manual_seed(29)
    pretrained_model = ToyDecoderModel(base_config)
    pretrained_optimizer = AdamW(pretrained_model.parameters(), lr=base_config.learning_rate)
    train(
        pretrained_model,
        pretrain_loader,
        base_config,
    ...
    sft_sequence = torch.arange(base_config.max_seq_len, dtype=torch.long).flip(0)
    sft_loader = DataLoader(
        _SFTDataset(sft_sequence, base_config.batch_size * 10),
        batch_size=base_config.batch_size,
        shuffle=False,
    )
